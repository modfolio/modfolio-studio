...
function gitRead(root: string, args: string[]): string | null {
  const r = spawnSync('git', args, { cwd: root, encoding: 'utf8', maxBuffer: GIT_READ_MAX_BUFFER });
...
export function contentTree(root: string): string {
  ...
  const g = (args: string[]) => spawnSync('git', args, { cwd: root, encoding: 'utf8', env });
  try {
    ...
    const rm = g(['rm', '-r', '--cached', '-q', '--ignore-unmatch', '--', ...excludePathspecs()]);
...
export function shouldKeepExisting(
  ...
  if (typeof existing.tree !== 'string' || existing.tree.length === 0) return false;
...
export function writeReceipt(
  ...
  if (
    existing !== null &&
    shouldKeepExisting(existing, { suite: receipt.suite, tree: receipt.tree ?? '' })
  ) {
    return existing;
  }
...
export function judgeReceipt(
  ...
  if (receipt === null) return { usable: false, why: '영수증이 없다 — 아직 완주한 적이 없다' };

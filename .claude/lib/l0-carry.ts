...
export function isL0Path(path: string, l0Paths: readonly string[]): boolean {
  return DOC_EXT.test(path) && l0Paths.some((l0) => path === l0 || path.startsWith(l0));
}
...
export function readL0Paths(root: string): string[] | null {
  for (const rel of ['.modfolio/ai-routing.json', 'config/ai-routing.json']) {
    ...
    try {
      const l0 = (JSON.parse(readFileSync(p, 'utf8')) as { reviewPolicy?: { l0Paths?: unknown } })
        .reviewPolicy?.l0Paths;
      if (Array.isArray(l0) && l0.length > 0 && l0.every((x) => typeof x === 'string'))
        return l0 as string[];
    } catch {
      // 다음 후보
    }
...
export const runPrivacyTest: PrivacyRunner = (root) => {
  ...
  return { ok: r.status === 0, detail: r.status === null ? '시간 초과' : `exit ${r.status}` };
};
...
export function judgeL0Carry(
  ...
  if (currentTree.length === 0) return no('지금 트리의 내용 정체를 못 구했다(git 실패)');
  ...
  const ns = git(root, ['diff', '--name-status', '-z', '--no-renames', base, currentTree]);
  ...
  try {
    ...
    allowed =
      decl.status === 0
        ? new Set(parseSecretAllowlist(decl.stdout ?? '').map((a) => `${a.file}::${a.id}`))
        : new Set();
  } catch {
  ...
  if (hits.length > 0)
    return no(`선언되지 않은 비밀 모양 ${hits.length}건 — ${hits.slice(0, 3).join(' · ')}`);

  // NUL — 추가·수정된 파일
  for (const e of entries.filter((x) => x.status !== 'D')) {
    const blob = spawnSync('git', ['cat-file', 'blob', `${currentTree}:${e.path}`], {
      cwd: root,
      maxBuffer: 64 * 1024 * 1024,
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    if (blob.status !== 0) return no(`${e.path} 를 못 읽었다`);
    if ((blob.stdout as Buffer).includes(0)) return no(`${e.path} 에 NUL 바이트가 있다`);
  }

  // 오너 원문 — 인계가 바뀌면 풀 게이트의 그 테스트를 그대로
  const handoff = entries.some(
    (e) => e.path === 'knowledge/HANDOFF.md' || e.path.startsWith('knowledge/handoff/'),
  );
  let privacy = '해당 없음(인계 변경 없음)';
  if (handoff) {
    if (!existsSync(join(root, PRIVACY_TEST)) || !existsSync(join(root, QUOTES_LEDGER)))
      privacy = '해당 없음(원장 없음)';
    else {
      ...
      for (const e of entries.filter(
        (x) => x.path === 'knowledge/HANDOFF.md' || x.path.startsWith('knowledge/handoff/'),
      )) {

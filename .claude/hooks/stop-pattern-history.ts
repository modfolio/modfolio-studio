...
const RULES: PatternRule[] = [
  {
    ...
    test: (file, content) => {
      ...
      for (const line of lines) {
        ...
        if (/#[0-9a-fA-F]{3,8}(?:[^a-zA-Z]|$)|rgb\(|hsl\(|oklch\(/.test(line)) return true;
      }
      return false;
    },
  },
  {
    id: 'ts_ignore_or_any',
    test: (file, content) => TS_EXT.test(file) && /@ts-ignore|as\s+any\b/.test(content),
  },
  {
    id: 'biome_ignore_file',
    test: (file, content) =>
      CODE_EXT.test(file) && /biome-ignore\s+(?:lint|assist)\s+file/.test(content),
  },
];
    ...
    for (const line of readFileSync(path, 'utf-8').split(/\r?\n/)) {
      ...
      try {
        ...
        rows.push({
          ...
          lastSeen: typeof parsed.lastSeen === 'string' ? parsed.lastSeen : today(),

...
function pipeExitTrap(cmd: string): boolean {
	...
	const forQuery = segments(stripCommandSubstitution(stripQuoted(cmd, "single")));
...
export function symlinkFalseZero(cmd: string): boolean {
	...
	for (const seg of cmd.split(/&&|\|\||[;|\n]/)) {
		...
		if (/(^|\s)-{1,2}[A-Za-z-]*\bL\b|(^|\s)--follow(\s|$)|(^|\s)-L(\s|$)/.test(t)) continue;
		return true;
	}
	return false;
}
...
function pkillSelf(cmd: string): boolean {
	...
	return /(^|[;&|(\s])pkill(?:\s+[^;&|\s]+)*\s+-[a-eg-z]*f[a-z]*(?=\s|$)/.test(stripQuoted(cmd));
}
...
function zshWordSplit(cmd: string): boolean {
	...
	for (const name of assigned) {
		...
		const setRe = new RegExp(`\\bset\\s+--\\s+\\$\\{?${name}\\}?(?![A-Za-z0-9_({=+:\\[])`);

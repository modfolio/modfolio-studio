...
export function biomeInvocation(
	...
	for (let i = 0; i < tokens.length; i++) {
		...
		flags.push(t);
		if (
			/^--(config-path|max-diagnostics|reporter|diagnostic-level|vcs-root)$/.test(t) &&
			tokens[i + 1]
		) {
...
export function feedback(files: readonly string[], output: string, limit = 1500): string {
	const body = output.trim();
	const clipped =
		body.length > limit ? `${body.slice(0, limit)}\n… (잘림 — 전체는 \`bun run check\`)` : body;
...
export function isViolation(
	...
	return typeof status === "number" && status !== 0 && files.some((f) => output.includes(f));
}
...
if (import.meta.main) {
	...
	const invocation = biomeInvocation(readCheckScript(cwd) ?? readCheckScript(top));

...
export function coveringLeases(cache: LeaseCache, rel: string, nowMs: number): string[] {
	...
	for (const it of cache.items ?? []) {
		const exp = typeof it.expiresAt === "string" ? Date.parse(it.expiresAt) : Number.NaN;
...
export function judge(file: string, sessionId: string, deps: JudgeDeps = {}): string[] {
	...
	if (status.ok && status.out !== "") {
		...
		const others = procs === null ? 0 : countOtherSessions(procs, process.pid, root);
		if (others > 0)
			warnings.push(

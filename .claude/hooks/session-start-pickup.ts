...
export function addressedToUs(
	...
	const sender =
		/^\s*(?:[-*]\s*)?\**\s*(?:발신|보낸 곳|보내는 곳|from)\s*\**\s*[:：]\s*(.+)$/im.exec(head);
	...
	for (const prefix of new Set([ownRepoName.toLowerCase(), short.toLowerCase()]))
		if (
...
export function recentInboxMessages(ecoRoot: string, ownRepoName: string): string[] {
...
export function buildUpdateCommand(root: string): string {
	...
	try {
		...
		const declared = Object.keys({ ...pkg.dependencies, ...pkg.devDependencies }).filter((d) =>
			d.startsWith("@modfolio/"),
		);
		return declared.length > 0 ? `bun update ${declared.sort().join(" ")}` : "bun update";
	} catch {

 ...
 */
function harnessAutoPull(root: string, installed: string, latest: string): string[] {
	...
	try {
		...
		execSync(`git commit -m "chore(harness): auto-pull ${latest} on session open"`, {
			cwd: root,
			stdio: "ignore",
			env,
		});
			...
			try {
				...
				if (installed && eco.harnessLatest) {
					if (compareSemver(installed, eco.harnessLatest) < 0) {
						...
						lines.push(...harnessAutoPull(gitRoot(), installed, eco.harnessLatest));
					}
				}
			} catch {

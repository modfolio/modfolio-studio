 ...
 */
export function otherClaudeSessions(procs: readonly ProcInfo[], myPid: number): ProcInfo[] {
...
export function countOtherSessions(
	...
	return otherClaudeSessions(procs, myPid).filter((p) => inside(p.cwd, root)).length;
}

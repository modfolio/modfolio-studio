...
const endpoint = process.env.OTEL_EXPORTER_OTLP_ENDPOINT ?? "http://otel.mod-ai.localhost";
		...
		try {
			...
			if (typeof parsed.pattern === "string") {
				rows.push({
					...
					status: typeof parsed.status === "string" ? parsed.status : "TRACKING",
				});
			}
		} catch {
			// skip malformed
	...
	resourceLogs: [
		{
			resource: {
				attributes: [
					...
					{ key: "repo", value: { stringValue: cwd.split(/[\\/]/).pop() ?? "unknown" } },
				],
			},
			scopeLogs: [
				{
					...
					logRecords: [
						{
							...
							attributes: [
								...
								{ key: "escalated_patterns", value: { intValue: escalated.length } },
								{ key: "total_patterns", value: { intValue: patterns.length } },
								{
									key: "escalated_list",
									value: {
										stringValue: escalated.map((r) => r.pattern).join(",") || "(none)",
									},
								},
							],
						},

...
export function stripHeredocs(cmd: string): { text: string; shellBodies: string[] } {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		const ops = [...line.matchAll(/<<(-?)\s*(?:'([^']+)'|"([^"]+)"|\\?([A-Za-z_][A-Za-z0-9_]*))/g)];
		for (const op of ops) {
			const dash = op[1] === "-";
...
export function isGitPushWords(w: readonly string[]): boolean {
	...
	for (;;) {
		...
		if (/^wsl(\.exe)?$/i.test(basename(h))) {
			...
			while (i < w.length) {
				const a = w[i] ?? "";
				if (/^(-d|--distribution|-u|--user|--cd|--shell-type)$/i.test(a)) i += 2;
	...
	if (rest.includes("--help") || rest.includes("-h") || rest.includes("--dry-run")) return false;
	return true;
}
...
export function isGitPushCommand(cmd: string): boolean {
	if (!cmd) return false;
	for (const words of simpleCommands(cmd)) if (isGitPushWords(words)) return true;
	return false;
}
...
const PUSH_OPT_UNREADABLE = /^(--all|--mirror|--tags|--branches|--follow-tags|--prune)$/;
 ...
 */
export function plainPushTargetsAll(cmd: string): readonly PushTargets[] | null {
...
function readPushWords(w: readonly string[]): PushTargets | null {
	...
	return { remote: positional[0] ?? null, refspecs: positional.slice(1), deletion, verify };
}
...
export function pushWorkdir(
	...
	for (const w of simpleCommands(cmd)) {
		...
		if (h === "cd" || h === "pushd") {
			...
			if (args.length > 1 || target === "-") known = false;
				...
				const next = target === undefined ? home : resolveDir(cwd, target, home);
		...
		if (!known) return { kind: "unknown", reason: "push 앞의 `cd` 대상을 실행 전에 알 수 없다" };
		const gi = w.findIndex((a) => basename(a) === "git");
		if (gi < 0) {
			// `bash -c "…"` · `eval …` — 본문을 지금 cwd 에서 다시 읽는다.
			const ci = w.findIndex((a) => /^-[A-Za-z]*c[A-Za-z]*$/.test(a));
			const body = ci >= 0 ? w[ci + 1] : w[0] === "eval" ? w.slice(1).join(" ") : undefined;
			if (body === undefined)
		...
		for (let i = gi + 1; i < w.length && w[i] !== "push"; i++) {
			...
			if (
				...
				return { kind: "unknown", reason: `\`${a}\` 로 저장소를 바꾼 push 는 읽지 않는다` };
			if (a === "-C") {
				const next = resolveDir(dir, w[i + 1] ?? "", home);
				if (next === null || (w[i + 1] ?? "") === "")
					return { kind: "unknown", reason: "`git -C` 대상을 실행 전에 알 수 없다" };
				dir = next;
				viaC = true;
				i++;
	...
	if (dirs.size > 1)
		return { kind: "unknown", reason: `push 들이 서로 다른 곳에서 돈다: ${[...dirs].join(" · ")}` };
	const only = [...dirs][0];

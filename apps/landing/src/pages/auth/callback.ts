...
export const GET: APIRoute = async ({ url, cookies, redirect }) => {
	...
	const verifier = cookies.get("ms_pkce_verifier")?.value;
	const returnTo = cookies.get("ms_return_to")?.value ?? "/";

	cookies.delete("ms_pkce_verifier", { path: "/" });
	cookies.delete("ms_oauth_state", { path: "/" });
	cookies.delete("ms_return_to", { path: "/" });
